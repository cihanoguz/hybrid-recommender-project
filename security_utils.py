...
def sanitize_file_path(
    file_path: Path, allowed_base: Optional[Path] = None
...
def validate_pickle_file_integrity(
    ...
    try:
        ...
        if file_size > max_file_size:
            return (
                False,
                f"File too large: {file_size / 1024 / 1024:.2f} MB (max: {max_file_size / 1024 / 1024:.2f} MB)",
            )
...
def sanitize_user_input(
    input_str: str, max_length: int = 1000, allowed_chars: Optional[str] = None
) -> str:
    """
    ...
    sanitized = "".join(
        char for char in input_str if ord(char) >= 32 or char == "\n" or char == "\t"
    )
...
def validate_numeric_input(
    value: float, min_value: float, max_value: float, param_name: str
) -> Tuple[bool, Optional[str]]:
    """
