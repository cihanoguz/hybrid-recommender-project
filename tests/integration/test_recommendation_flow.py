...
class TestHybridFlow:
    ...
    def test_hybrid_combines_all_approaches(
        self,
        sample_user_ids,
        sample_movie_data,
        sample_rating_data,
        ...
        pre_u = precompute_for_user_userbased(
            1, sample_user_ids, sample_user_movie_df, sample_rating_data
        )
        if pre_u["status"] == "ok":
        ...
        pre_i = precompute_for_user_itembased(
            1, sample_user_ids, high_ratings, sample_movie_data, sample_user_movie_df
        )
        if pre_i["status"] == "ok":
        ...
        # Content-based
        cosine_sim = np.eye(len(sample_movie_data))
        result_cb = content_based_recommender_cached("Movie A", 5, sample_movie_data, cosine_sim)
