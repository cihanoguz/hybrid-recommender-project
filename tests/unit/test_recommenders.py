...
class TestUserBasedRecommender:
    ...
    def test_precompute_for_user_userbased_invalid_user(
        self, sample_user_ids, sample_user_movie_df, sample_rating_data, mock_streamlit_cache
    ):
    ...
    def test_precompute_for_user_userbased_no_movies(
        self, sample_user_ids, sample_rating_data, mock_streamlit_cache
    ):
    ...
    def test_precompute_for_user_userbased_valid(
        self, sample_user_ids, sample_user_movie_df, sample_rating_data, mock_streamlit_cache
    ):
...
class TestItemBasedRecommender:
    ...
    def test_precompute_for_user_itembased_invalid_user(
        self,
        sample_user_ids,
        sample_movie_data,
        sample_rating_data,
        sample_user_movie_df,
        mock_streamlit_cache,
    ):
        """Test precompute with invalid user ID."""
    ...
    def test_precompute_for_user_itembased_no_five_star(
        self, sample_user_ids, sample_movie_data, sample_user_movie_df, mock_streamlit_cache
    ):
        """Test precompute for user with no 5-star ratings."""
        from recommenders.item_based import precompute_for_user_itembased

        low_ratings = pd.DataFrame(
            {"userId": [1, 1], "movieId": [1, 2], "rating": [3.0, 4.0], "timestamp": [1000, 2000]}
        )
    ...
    def test_precompute_for_user_itembased_valid(
        self, sample_user_ids, sample_movie_data, sample_user_movie_df, mock_streamlit_cache
    ):
        ...
        high_ratings = pd.DataFrame(
            {"userId": [1, 1], "movieId": [1, 2], "rating": [5.0, 4.5], "timestamp": [1000, 2000]}
        )
...
class TestContentBasedRecommender:
    ...
    def test_content_based_recommender_cached_movie_not_found(
        self, sample_movie_data, mock_streamlit_cache
    ):
    ...
    def test_content_based_recommender_cached_valid(self, sample_movie_data, mock_streamlit_cache):
        ...
        result = content_based_recommender_cached(
            movie_title="Movie A", top_n=5, movie=sample_movie_data, cosine_sim_genre=cosine_sim
        )
