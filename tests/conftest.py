...
def sample_pickle_data(sample_movie_data, sample_rating_data, sample_user_movie_df):
    ...
    return {
        ...
        'df_full': pd.concat([sample_rating_data, sample_movie_data.merge(
            sample_rating_data, on='movieId', how='inner'
        )], ignore_index=True),
...
def mock_streamlit(monkeypatch):
    ...
    def mock_columns(n):
        if isinstance(n, list):
            ...
            num_cols = n
        return [type('Col', (), {'__enter__': lambda s: s, '__exit__': lambda s, *a: None})() for _ in range(num_cols)]
    mock_st.columns = mock_columns
    mock_st.markdown = lambda *args, **kwargs: None
    mock_st.tabs = lambda names: [type('Tab', (), {'__enter__': lambda s: s, '__exit__': lambda s, *a: None})() for _ in names]
    ...
    mock_st.selectbox = lambda *args, **kwargs: args[1][0] if isinstance(args[1], list) and args[1] else None
    ...
    mock_st.expander = lambda *args, **kwargs: type('Expander', (), {'__enter__': lambda s: s, '__exit__': lambda s, *a: None})()
    mock_st.spinner = lambda *args, **kwargs: type('Spinner', (), {'__enter__': lambda s: s, '__exit__': lambda s, *a: None})()
...
def reset_environment(monkeypatch):
    ...
    env_vars_to_clear = [
        'LOG_LEVEL', 'LOG_FILE', 'PICKLE_PATH', 'LOGO_PATH',
        'DEFAULT_USER_ID', 'SERVER_PORT'
    ]
